resource "aws_ecr_repository" "jumbo" {
  ...
  provisioner "local-exec" {
    ...
    command = "aws ecr get-login-password --region ${var.aws_region} | docker login --username AWS --password-stdin ${aws_ecr_repository.jumbo.repository_url} && docker build -t ${var.app_name} -f ../JumboServer/Dockerfile.webserver ../JumboServer/. && docker tag ${var.app_name}:latest ${aws_ecr_repository.jumbo.repository_url}:latest && docker push ${aws_ecr_repository.jumbo.repository_url}:latest"
  }
}
